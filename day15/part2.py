...
with open(file) as f:
    ...
    matrix = [[x for x in range(len(matrix)*5)] for row in range(len(matrix)*5)]
    print(base_matrix)
    print(matrix)
    for i in range(5):
        for j in range(5):
            for k in range(len(base_matrix)):
                for l in range(len(base_matrix[k])):
                    value = int(base_matrix[k][l]) + i + j
                    if value > 9:
                        value = value - 9
                    print(f"{i} {j} {k} {l} {value}")
                    i_pos = i*len(base_matrix)
                    j_pos = j*len(base_matrix[k])
                    matrix[i_pos+k][j_pos+l] = value
